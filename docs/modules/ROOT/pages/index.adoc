...
Then you can iterate over the list to get all the information about the detected supportedEntities that could be anonymized.
...
Also, a high level class is provided to simplify the flow of calling analyzer and then anonymizer.
Same example can be rewritten to:
...
public class PresidioServicePipeline {
    ...
    public PresidioServicePipeline() {
        this.presidioPipeline = PresidioPipeline.builder() // <1>
    ...
    public String analysisAndAnonymize(String text, String language) {
        return this.presidioPipeline.process(text, language); // <2>
    }

}
----
<1> Automatically injects the Analyzer and Anonymizer classes
<2> Runs all the Presido flow
